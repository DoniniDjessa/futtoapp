...
| 2026-09-23 | **P0.3** | Effectif match : `futto_join_match` durci (un match privé ne se rejoint que si ouvert `public`/`both`, invité `invited`, ou hôte — avant : tout user connecté par id/lien). RLS insert/update `futto_match_players` bloquées hors RPC (plus d'auto-`joined` par upsert direct ; joueur ne peut que `left`). `duplicate()` de `match/[id]` auto-inscrit l'hôte (comme `creer`). Sync `spots_taken` déjà géré par trigger `futto_match_players_spots_aiud` |
| 2026-09-23 | **P0.4** | Inviter WhatsApp : le lien de partage `https://futto.app/m/{token}` est maintenant fonctionnel. Nouvelle route publique `app/m/[token].tsx` (résolution par RPC definer `futto_match_by_token`, visible avant login, matchs privés inclus) → carte invitation + bouton « Rejoindre ce match » (`futto_join_match` étendu avec `p_token` = invitation). Home : bouton « Inviter » sur la card prochain match (message prérempli via `matchInviteMessage`). Message déjà complet (titre/lieu/heure/places/prix/lien) |
