-- Migre anciennes tables → préfixe futto_
-- À lancer UNE FOIS si profiles / terrains existent déjà.
...
begin
  if exists (
    ...
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'futto_profiles'
  ) then
  ...
  if exists (
    ...
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'futto_terrains'
  ) then
  ...
  if exists (
    ...
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'futto_matches'
  ) then
  ...
  if exists (
    ...
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'futto_match_players'
  ) then
  ...
  if exists (
    ...
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'futto_wallet_accounts'
  ) then
  ...
  if exists (
    ...
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'futto_wallet_transactions'
  ) then
  ...
  if exists (
    ...
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'futto_notifications'
  ) then
  ...
  if exists (
    ...
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'futto_tournaments'
  ) then
    alter table public.tournaments rename to futto_tournaments;
  end if;
end $$;

-- Colonne pseudo + contrainte rôle
...
-- Recréer trigger + helpers (pointent vers futto_*)
